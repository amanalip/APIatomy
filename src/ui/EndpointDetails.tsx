...
export const EndpointDetails: React.FC<EndpointDetailsProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 border-l border-slate-800 text-slate-100 overflow-hidden shadow-2xl">
      {/* Header */}
      <div className="p-4 border-b border-slate-800 bg-slate-950/70 flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            ...
            {endpoint.deprecated && (
              <span className="px-2 py-0.5 text-[10px] font-medium rounded bg-red-500/20 text-red-300 border border-red-500/40">
                Deprecated
              </span>
            )}
          </div>
      ...
      {/* Tabs */}
      <div className="flex items-center gap-2 px-4 border-b border-slate-800 bg-slate-950/40">
        <button
          ...
          <>
                ...
                <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-2">
                  {endpoint.security.map((sec, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs">
                      <span className="font-mono text-amber-300 font-medium">{sec.name}</span>
                      {sec.scopes.length > 0 ? (
            ...
            <div className="space-y-2">
              ...
              {endpoint.parameters.length === 0 ? (
                <div className="text-xs text-slate-500 italic p-3 rounded-lg bg-slate-950/40 border border-slate-800/60">
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {endpoint.parameters.map((p) => (
                        <tr key={`${p.in}-${p.name}`} className="hover:bg-slate-900/40">
                          ...
                          <td className="py-2.5 px-3 text-slate-400 text-[11px]">
                            {p.description || <span className="italic text-slate-600">No description</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-300">
                  ...
                  {endpoint.requestBody.required && (
                    <span className="text-[9px] px-1 py-0.2 rounded bg-red-500/20 text-red-400 font-bold ml-1">
                      Required
                    </span>
                  )}
                </div>
                ...
                <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800 space-y-3">
                  {endpoint.requestBody.description && (
                    <p className="text-xs text-slate-400">{endpoint.requestBody.description}</p>
                  )}

                  {endpoint.requestBody.content.map((c, i) => (
                    <div key={i} className="space-y-2">
                      <div className="flex items-center gap-2">
                        <span className="px-2 py-0.5 rounded bg-blue-950/60 text-blue-300 border border-blue-800/60 text-[11px] font-mono">
                          {c.contentType}
                        </span>
                        ...
                        {c.schema?.refTarget && (
                          <button
                            ...
                            className="flex items-center gap-1 text-[11px] text-indigo-400 hover:text-indigo-300 font-mono underline"
                          >
                      ...
                      {c.schema && (
                        <div className="p-2.5 rounded-lg bg-slate-900 border border-slate-800 font-mono text-xs text-slate-300 overflow-x-auto">
                          <SchemaPropertyTree
                            schema={c.schema}
                            schemas={schemas}
                  ...
                  return (
                    ...
                    >
                      <button
                        ...
                        className="w-full px-3 py-2 flex items-center justify-between hover:bg-slate-900/60 transition text-left"
                      >
                            ...
                            <div key={idx} className="space-y-2">
                              {c.schema?.refTarget && (
                                <button
                                  ...
                                  className="flex items-center gap-1 text-xs text-indigo-400 hover:text-indigo-300 font-mono underline"
                                >
                                  <span>Schema: {c.schema.refTarget}</span>
                                  <ArrowRight className="w-3 h-3" />
                                </button>
        ...
        {Object.entries(schema.properties).map(([propName, propSchema]) => {
          ...
          return (
            <div key={propName} className="flex items-baseline gap-2 py-0.5 text-xs">
              <span className="text-slate-200 font-semibold">{propName}</span>
              {isRequired && <span className="text-red-400 text-[10px] font-bold">*</span>}
              ...
              {propSchema.refTarget && (
                <button
                  ...
                  className="text-indigo-400 hover:text-indigo-300 text-[11px] underline flex items-center gap-0.5"
                >
  ...
  if (schema.items) {
    return (
      <div className="text-xs text-slate-300">
        ...
        <span className="text-blue-400 font-semibold">{String(schema.items.type || 'object')}</span>
