...
export const EditorPane = forwardRef<EditorPaneRef, EditorPaneProps>(
  ({ value, onChange, format = 'yaml' }, ref) => {
    ...
    return (
      <div
        className={`relative flex flex-col h-full bg-slate-950 text-slate-100 overflow-hidden ${
          isDragging ? 'ring-2 ring-blue-500 bg-blue-950/20' : ''
        }`}
        onDragOver={(e) => {
          e.preventDefault();
          setIsDragging(true);
      ...
      >
        ...
        {/* Drag Overlay */}
        {isDragging && (
          <div className="absolute inset-0 bg-blue-950/80 backdrop-blur-sm flex flex-col items-center justify-center pointer-events-none z-30">
            <Upload className="w-12 h-12 text-blue-400 mb-2 animate-bounce" />
            <p className="text-sm font-medium text-white">Drop your OpenAPI or Swagger spec file here</p>
            <p className="text-xs text-blue-300">Accepts .yaml, .yml, .json</p>
          </div>
        )}
      </div>
    );
