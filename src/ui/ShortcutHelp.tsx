...
export const ShortcutHelp: React.FC<ShortcutHelpProps> = ({ onClose }) => {
  ...
  return createPortal(
    ...
    >
      <div
        ref={trapRef}
        className="w-full max-w-md rounded-xl bg-white dark:bg-slate-900 border-2 border-blue-500 shadow-2xl overflow-hidden ring-4 ring-blue-500/20"
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
          ...
          <button
            ref={closeRef}
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 border border-transparent hover:border-slate-200 dark:hover:border-slate-600 text-slate-700 dark:text-slate-300"
            aria-label="Close"
          >
            <X className="w-4 h-4" />
          </button>
